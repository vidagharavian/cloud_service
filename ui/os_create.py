...
class CreateOS(QtWidgets.QMainWindow):
    def __init__(self, user_id: int = None, os_id: int = None):
        ...
        self.base_bandwidth = self.findChild(QtWidgets.QDoubleSpinBox, 'bandwidth')
    ...
    def addButtonPressed(self):
        ...
        create_os(name=self.get_value(self.os_name), base_RAM=self.get_value(self.base_ram),base_CPU= self.get_value(self.base_cpu),
                  base_CORE=self.get_value(self.base_core), base_cost=self.get_value(self.cost), base_band_width=self.get_value(self.base_bandwidth))
        from ui.os_list import OSList
        self.OtherWindow = OSList(user_id=self.user_id)
        self.OtherWindow.show()
        self.close()
